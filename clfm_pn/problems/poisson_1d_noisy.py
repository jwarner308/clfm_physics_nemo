...
from clfm_pn.problems.poisson_1d import (
    Poisson1DDataset,
    Poisson1DLoss,
    v_func,
    f_func,
)
from clfm_pn.utils.utils import grad
...
class NoisyPoisson1DLoss(Poisson1DLoss):
    ...
    def validate(self, vae, *args):
        ...
        with torch.no_grad():
            ...
            u_noisy = self.dataset.u_samples_noisy.to(vae._bias.device)
            x_in = (
                x_sensor.unsqueeze(0)
                .expand(u_noisy.shape[0], -1, -1)
                .to(vae._bias.device)
            )
