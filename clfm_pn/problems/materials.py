...
from clfm_pn.utils import grad, reparameterize
...
class MaterialsLoss(nn.Module):
    ...
    def validate(self, vae, u: Tensor, x: Tensor, f: Tensor):
        ...
        val_loss_observable = torch.mean(
            torch.square(ux_pred - ux_true)
        ) + torch.mean(torch.square(uy_pred - uy_true))
        ...
        A_points = (
            torch.stack(
                [torch.linspace(0.0, 1.0, 25), torch.full((25,), y_line_A)],
                dim=1,
            )
            .unsqueeze(0)
        ...
        B_points = (
            torch.stack(
                [torch.linspace(0.0, 1.0, 25), torch.full((25,), y_line_B)],
                dim=1,
            )
            .unsqueeze(0)
            .repeat(x.shape[0], 1, 1)
        ...
        C_points = (
            torch.stack(
                [torch.linspace(0.0, 1.0, 25), torch.full((25,), y_line_C)],
                dim=1,
            )
            .unsqueeze(0)
            .repeat(x.shape[0], 1, 1)
